...
function Signup() {
  ...
  return (
    <motion.div 
        ...
        <div className='p-8'>
            <h1 className='text-3xl font-bold mb-6 text-center bg-center bg-gradient-to-r from-green-400 to-emerald-500 text-transparent bg-clip-text'>Create Account</h1>
            <form onSubmit={handleSignup}>
                 ...
                 {error && <p className='text-red-500 font-semibold mt-2'>{error}</p>} 
                    ...
                    whileTap={{scale:0.98}}
                    type='submit'
                    disabled = {isLoading}>{isLoading ? <Loader className=' animate-spin mx-auto' size={24} /> : "Sign Up"}</motion.button>
            </form>
            
        </div>

        <div className='px-8 py-4 bg-gray-900 bg-opacity-50 flex justify-center'>
                <h6 className='text-sm text-gray-400'>Already have an account? {" "}
                <Link to={"/login"} className='text-green-400 hover:underline'>Log in</Link></h6>
            </div>
